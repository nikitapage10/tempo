...
- Changed (v0.136.0): invitation emails are leaner and more clearly from TEMPO — quieter layout, a reply address so people can answer, and Admin warns when invite links sit on a different domain than the From address (a common reason Gmail junks them). Prefer sending from the same domain as the live site, or host the app under the sending domain — see the invitation email notes in DEPLOYMENT.
