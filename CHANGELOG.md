...
- Fixed (v0.220.1): adding someone to your Top 8 actually sticks now. Tapping a follower in the picker used to lose the race against the “click outside to close” handler, so the list closed and nothing was saved — it looked like the names were not clickable. The pick is taken the moment you press, shows up immediately, and if saving fails you get a clear message instead of silence.
- Added (v0.220.0): changing your @handle now tells you whether it is free while you type. On Artist → Edit profile (and the Pro Profile editor), a taken name turns the line under the field red as soon as you pause typing, instead of waiting until Save. The same live check lives under Settings → Account → Handle, so you can change it without opening the full profile. Save stays off until the name is valid and available.
- Added (v0.219.0): the TEMPO Pulse email now actually reports on your work. Until now it could only tell you one thing — how many unread messages you had — so if you read your messages in the app, it had nothing to say and sent nothing, which is why it looked broken. A digest now arrives in sections: tracks that need attention, tasks overdue or due, new feedback on your work, collaboration and team activity, what is on the calendar, unread messages, and, in the weekly one, a short look back at what you finished. Overdue work is separated from upcoming work and listed first, because those are not the same news. "Due today" now means today where you are, not where the server is.
- Fixed (v0.219.0): the seven category switches under Settings → Notifications do something now. They were saved to your account but nothing ever read them, so turning one off changed nothing about what arrived. Each one now removes that section from the email, and turning all of them off means nothing is sent.
- Changed (v0.219.0): Pulse emails still describe things by count only — "2 tasks are overdue" — unless you turn on names, and even then they never include what a message or a comment actually said. A digest only ever covers workspaces you own, not one shared with you by another artist, and never the demo artist's sample deadlines.
  - Note: email Pulse is still off until you switch it on in Settings → Notifications; that has not changed. In-app Pulse on Today is unaffected and still covers unread messages only.
- Changed (v0.218.0): Messages only lists the kinds of conversation you actually have. TEMPO Support, Teams, Sessions, Groups, and Artist messages each used to hold a heading and a sentence explaining what would go there one day, so a single real conversation sat at the bottom of four empty rooms. Each heading now appears with its first conversation and disappears with its last. An inbox with nothing in it at all still says so, in one line that names what will show up there.
- Changed (v0.217.0): choosing to join the network no longer picks your handle for you. The box used to open already filled in with a version of your artist name, so pressing Continue claimed it before you had read it — and a handle is permanent and public. The box now starts empty and says so, with your artist name offered beside it as a one-tap suggestion if that is genuinely what you want. Pressing Continue with nothing typed tells you in red what is missing and puts the cursor in the field, instead of quietly refusing. Same in the Join the network box on Social and the Artist page; a handle you already hold still appears, because that one is yours.
- Removed (v0.216.0): you were right — your friend never wrote that. When a new member first arrived, TEMPO quietly posted two welcome messages in the Green Room feed and two more in its chat, and signed them with real members' names: whoever invited them, or someone on the team. Those people were never asked and never saw it happen, and everyone else in the room read the words as genuinely theirs. TEMPO no longer writes anything as anybody. The Green Room now opens with its welcome checklist and an empty feed, and fills up only when actual members say something.
  - Under the hood: run migration 119 in Supabase to delete the four messages already sitting in the room. It only removes those exact four sentences — nothing anyone actually wrote is touched.
- Changed (v0.216.0): TEMPO no longer changes anyone's privacy setting on their behalf. The same first-run step used to take the person who sent the invite, or someone on the team, and switch their profile from private to visible to all TEMPO members, so the new arrival had a face to see. Nobody was asked, and the owner was not there when it happened. A private profile now stays private and is simply not used as a welcome face.
- Fixed (v0.215.2): the Scenes and Social tours work on the demo artist. Those two pages show a different, read-only version of themselves when you are in PRESIDENT, and the tour had only been taught where things are on the normal version — so on the demo it pointed at the page in general. It now points at the sample scene and the sample network, and the step about posting is left out there, because there is nothing on the demo to post with. The Board tour also works in the all-stages overview, not just the focused view, and any step whose subject is missing — no tracks yet, no profile story yet, a Social page you have not joined — is now dropped instead of pointing at the nearest thing on screen.
- Fixed (v0.215.1): the page tours now point at the right thing on every screen, not just on Sessions. Each step used to describe one part of the page and then guess which part that was, which is why Stats opened by circling the artist name instead of the numbers, and why steps elsewhere landed on a filter row or the notification bell. Every step now points at the exact element it is talking about — the summary on Stats, the stage columns on Board, the capture box on Tasks, the composer on Social, the calendar itself on Calendar — and a step about something you do not have yet is left out instead of pointing somewhere random.
- Fixed (v0.214.4): the Sessions tour now highlights the New session button when it says “Make a room, invite the people.” It was pointing at the notification bell in the top bar, which has nothing to do with starting a session. Every page tour now only ever highlights something on the page itself, never the toolbar or the banners above it.
- Fixed (v0.214.4): “The room lives here” was highlighting the Show/Active/Archived filters on a Sessions page that had no sessions in it. That step now points at an actual Session, and a tour step about something you do not have yet is left out entirely rather than pointing at the nearest thing on screen.
- Fixed (v0.214.4): a project with a lot of attached tracks no longer pushes Release and Readiness far down the page. The track list on the right now scrolls once it gets long instead of stretching the whole page with it.
- Fixed (v0.214.3): Origin and Passage play the Tempo Theme bed again when you Tune in. A tiny timing quirk in the fade-up could throw on the first frame and leave the music running silently underneath the film; that fade now stays in a legal volume range so the bed actually comes up.
...
- Fixed (v0.198.1): Rearranging on Tracks now uses the same drag as the Board — the list stays still while you hold a row, and on drop that row slides into place without the stuttering, laggy motion.
- Fixed (v0.198.1): On the Board, a track dropped between two others no longer flies in from the top or bottom of the column before settling in the gap you aimed at.
