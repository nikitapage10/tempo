...
export default function MessagesView() {
  ...
  /**
   * A heading with an explanation of what would be there one day is not an
   * inbox, it is a brochure — and four of them pushed the one real
   * conversation to the bottom of the list. Each kind of conversation appears
   * only once it exists; when none of them do, a single line below covers it.
   */
  const inboxLoading = isLoading || support.isLoading;
  const inboxCount =
    ...
    <div className={cn("grid gap-3 lg:grid-cols-[19rem_minmax(0,1fr)]", MESSAGES_WORKSPACE_HEIGHT_CLASS)}>
      <aside className={cn("panel-quiet min-h-0 overflow-hidden", showingThread && "hidden lg:flex", "flex-col")}>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          {support.isLoading ? <div className="h-16 animate-pulse bg-bg-2/40"/> : visibleSupport.length ? <><div className="border-b border-line px-3 py-2"><p className="label-mono flex items-center gap-2"><Headphones className="size-3.5 text-amber"/>TEMPO Support</p></div><ul>{visibleSupport.map((thread) => { const unread = Boolean(thread.last_admin_reply_at && (!thread.member_last_read_at || thread.last_admin_reply_at > thread.member_last_read_at)); return <li key={thread.id}><button type="button" onClick={() => chooseSupport(thread.id)} className={cn("flex w-full items-start gap-2.5 border-b border-line px-3 py-3 text-left hover:bg-bg-2", activeSupportId === thread.id && "bg-amber/[0.07]")}><span className="mt-0.5 flex size-7 shrink-0 items-center justify-center rounded-full border border-amber/25 bg-amber/10"><Headphones className="size-3.5 text-amber"/></span><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className="truncate text-sm text-text-hi">{thread.subject}</p>{unread ? <span className="ml-auto size-2 rounded-full bg-amber"/> : null}</div><p className="mt-0.5 truncate text-xs text-text-lo">{thread.messages.at(-1)?.body || (thread.messages.at(-1)?.media?.length ? "Sent an attachment" : thread.details)}</p></div></button></li>; })}</ul></> : null}
          {teamConversations.length ? <><div className="border-b border-line px-3 py-2"><p className="label-mono flex items-center gap-2"><Users className="size-3.5 text-ice"/>Teams</p></div><ul>{teamConversations.map((conversation) => <li key={conversation.id}><InboxRow conversation={conversation} active={activeId === conversation.id} onChoose={chooseDirect} team/></li>)}</ul></> : null}
          {sessionConversations.length ? <><div className="border-b border-line px-3 py-2"><p className="label-mono flex items-center gap-2"><Radio className="size-3.5 text-ice"/>Sessions</p></div><ul>{sessionConversations.map((conversation) => <li key={conversation.id}><InboxRow conversation={conversation} active={activeId === conversation.id} onChoose={chooseDirect} session/></li>)}</ul></> : null}
          {groupConversations.length ? <><div className="border-b border-line px-3 py-2"><p className="label-mono flex items-center gap-2"><MessagesSquare className="size-3.5 text-ice"/>Groups</p></div><ul>{groupConversations.map((conversation) => <li key={conversation.id}><InboxRow conversation={conversation} active={activeId === conversation.id} onChoose={chooseDirect} group/></li>)}</ul></> : null}
          {directConversations.length ? <><div className="border-b border-line px-3 py-2"><p className="label-mono">Artist messages</p></div><ul>{directConversations.map((conversation) => <li key={conversation.id}><InboxRow conversation={conversation} active={activeId === conversation.id} onChoose={chooseDirect}/></li>)}</ul></> : null}
          ...
          {inboxLoading ? <div className="h-24 animate-pulse bg-bg-2/40"/> : inboxCount === 0 ? <p className="p-3 text-xs leading-relaxed text-text-lo">{inboxQuery ? "No matching conversations." : archived ? "Nothing archived yet." : "No conversations yet. Artist messages, group chats, Session chats, team rooms, and anything you send to TEMPO Support all arrive here."}</p> : null}
        </div>
      </aside>
