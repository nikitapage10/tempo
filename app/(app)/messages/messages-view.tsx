...
import { Archive, ArchiveRestore, ChevronLeft, Files, Headphones, Lock, MessagesSquare, PenSquare, Pin, Search, Trash2, Users, Volume2, VolumeX, X } from "lucide-react";
...
import { conversationHeading, conversationMemberLabel, isArtistGroupConversation } from "@/lib/messages/behavior";
...
export default function MessagesView() {
  ...
  React.useEffect(() => { setReplyTo(null); setSupportReplyTo(null); setUtility(null); setMessageQuery(""); setShowMembers(false); }, [activeId, activeSupportId]);
  ...
  const groupConversations = visibleConversations.filter((conversation) => isArtistGroupConversation({ kind: conversation.kind, teamArtistId: conversation.team_artist_id }));
  ...
  const activeIsGroup = Boolean(active && isArtistGroupConversation({ kind: active.kind, teamArtistId: active.team_artist_id }));
  const activeHeading = activeSupport
    ? `TEMPO Support / ${activeSupport.subject}`
    : active
      ? conversationHeading({ kind: active.kind, title: active.title, teamArtistId: active.team_artist_id, peerName: active.peer?.display_name })
      : "Conversation";
...
function InboxRow({ conversation, active, onChoose, team = false, group = false }: { conversation: Conversation; active: boolean; onChoose: (id: string) => void; team?: boolean; group?: boolean }) {
  const heading = conversationHeading({ kind: conversation.kind, title: conversation.title, teamArtistId: conversation.team_artist_id, peerName: conversation.peer?.display_name });
  const preview = conversation.last_message_preview ?? (team ? "Team room" : group ? "Group chat" : "No messages yet");
