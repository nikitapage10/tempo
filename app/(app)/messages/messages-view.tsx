...
import { Archive, ArchiveRestore, Headphones, Lock, MessagesSquare, Trash2, Users } from "lucide-react";
...
export default function MessagesView() {
  ...
  const { data: conversations = [], isLoading } = useConversations(onNetwork ? myProfileId : null, archived);
  ...
  React.useEffect(() => { const directId = searchParams.get("c"); const supportId = searchParams.get("support"); if (directId) { setActiveId(directId); setActiveSupportId(null); } else if (supportId) { setActiveSupportId(supportId); setActiveId(null); } }, [searchParams]);
  React.useEffect(() => {
    if (activeId && onNetwork && !activeSupportId) mutations.markRead.mutate(activeId);
    // eslint-disable-next-line react-hooks/exhaustive-deps -- mark once when a thread opens
  }, [activeId, onNetwork, activeSupportId]);

  function chooseDirect(id: string) { setActiveId(id); setActiveSupportId(null); }
  ...
  return <div className="space-y-5">
    <PageHeader title="Messages" subtitle="Artist conversations and private help from TEMPO Support." actions={<div className="flex rounded-input border border-line bg-bg-1 p-1"><button type="button" onClick={() => switchArchive(false)} className={cn("rounded-md px-3 py-1.5 text-xs", !archived ? "bg-ice text-bg-0" : "text-text-lo")}>Inbox</button><button type="button" onClick={() => switchArchive(true)} className={cn("rounded-md px-3 py-1.5 text-xs", archived ? "bg-ice text-bg-0" : "text-text-lo")}>Archived</button></div>}/>
