    ...
    {data ? <>
      <section className="glass-hero prism-edge relative overflow-hidden">
        <div className="grid gap-6 p-6 md:grid-cols-[1fr_auto] md:p-7">
          <div>
            <p className="label-mono text-violet">Last <span className="tabular-nums">{period}</span> days</p>
            <h2 className="mt-3 font-display text-2xl font-semibold text-text-hi">
              <span className="tabular-nums">{sum("aiMessages").toLocaleString()}</span> AI requests and{" "}
              <span className="tabular-nums">{sum("uploads").toLocaleString()}</span> uploads
            </h2>
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-text-lo">
              Members added <span className="tabular-nums">{bytes(sum("storageAddedBytes"))}</span> of new material and completed{" "}
              <span className="tabular-nums">{hours(sum("focusSeconds"))}</span> of focused work in this window.
            </p>
          </div>
          <div className="flex items-center gap-3 md:pr-3">
            <span className="flex size-12 items-center justify-center rounded-full border border-violet/20 bg-violet/10">
              <WandSparkles className="size-5 text-violet" />
            </span>
          </div>
        </div>
      </section>
