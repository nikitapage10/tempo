...
export default function AdminInvitesPage() {
  ...
  return <div className="space-y-5">
    ...
    <Dialog open={open} onOpenChange={setOpen}><DialogContent title="Create invitation" description="Add an email to send TEMPO’s invitation automatically. Leave it blank to create a copyable link." onClose={() => setOpen(false)}><div className="space-y-3">
      ...
      {email ? <div className="well rounded-input p-4"><p className="label-mono text-amber">Email preview</p><p className="mt-3 font-display text-lg text-text-hi">{memberRole === "team_member" ? "Step into the work around the music." : "Bring your music into focus."}</p><p className="mt-1 text-xs leading-relaxed text-text-lo">{memberRole === "team_member" ? <>The invitation identifies them as a <span className="text-text-hi">pro</span>, explains Passage, their professional home, a tour built for their work, and direct access to Nikita, then includes their code and both ways to open TEMPO.</> : <>The invitation identifies them as a <span className="text-text-hi">{ROLE_OPTIONS.find((option) => option.value === memberRole)?.label.toLowerCase()}</span>, explains Origin, the tour, starter checklist, and direct access to Nikita, then includes their code and both ways to open TEMPO.</>}</p>{welcomeNote.trim() ? <div className="mt-3 border-l-2 border-ice pl-3 text-xs leading-relaxed text-text-hi">{welcomeNote}</div> : null}</div> : null}
