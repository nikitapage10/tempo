...
import { Archive, ArchiveRestore, Bug, CheckCircle2, Clock3, LifeBuoy, Trash2, UserRound } from "lucide-react";
...
export default function AdminSupportPage() {
  ...
  return <div className="space-y-5">
    <PageHeader title="Support inbox" subtitle="Private member conversations for bugs, questions, and product feedback." actions={<div className="flex rounded-input border border-line bg-bg-1 p-1"><button type="button" onClick={() => { setArchived(false); setSelectedId(null); }} className={cn("rounded-md px-3 py-1.5 text-xs", !archived ? "bg-ice text-bg-0" : "text-text-lo")}>Inbox</button><button type="button" onClick={() => { setArchived(true); setSelectedId(null); }} className={cn("rounded-md px-3 py-1.5 text-xs", archived ? "bg-ice text-bg-0" : "text-text-lo")}>Archived</button></div>}/>
    <div className="flex flex-wrap items-center gap-1.5">{statuses.map((value) => <button key={value || "all"} type="button" onClick={() => { setStatus(value); setSelectedId(null); }} className={cn("rounded-chip border px-3 py-1.5 text-xs capitalize transition-colors", status === value ? "border-ice/40 bg-ice/10 text-ice" : "border-line text-text-lo hover:border-ice/20 hover:text-text-hi")}>{value ? value.replace("_", " ") : "All tickets"}</button>)}{reports.length ? <span className="ml-auto text-xs tabular-nums text-text-lo">{reports.length} {reports.length === 1 ? "ticket" : "tickets"}</span> : null}</div>
    ...
    {selected ? <div className="grid min-h-[34rem] gap-4 lg:grid-cols-[20rem_minmax(0,1fr)]">
      <aside className="panel overflow-hidden"><div className="border-b border-line px-4 py-3"><p className="label-mono">Ticket queue</p></div><div className="max-h-[42rem] overflow-y-auto">{reports.map((report) => { const Icon = report.category === "bug" ? Bug : LifeBuoy; const preview = report.messages.at(-1)?.body || (report.messages.at(-1)?.media?.length ? "Sent an attachment" : report.details); const lastMember = [...report.messages].reverse().find((message) => message.sender_role === "member")?.created_at ?? report.created_at; const unread = !report.admin_last_read_at || lastMember > report.admin_last_read_at; return <button key={report.id} type="button" onClick={() => setSelectedId(report.id)} className={cn("w-full border-b border-line px-4 py-3 text-left transition-colors hover:bg-bg-2", selected.id === report.id && "bg-ice/[0.07]")}><div className="flex items-center gap-2"><Icon className={cn("size-3.5", report.category === "bug" ? "text-warn" : "text-ice")}/><span className="label-mono truncate text-[10px]">{report.category}</span>{unread ? <span className="ml-auto size-2 rounded-full bg-amber"/> : <span className="ml-auto text-[10px] text-text-lo">{report.status.replace("_", " ")}</span>}</div><p className="mt-2 truncate text-sm font-medium text-text-hi">{report.subject}</p><p className="mt-1 line-clamp-2 text-xs leading-relaxed text-text-lo">{preview}</p><p className="mt-2 flex items-center gap-1 text-[10px] text-text-lo"><Clock3 className="size-3"/>{date(report.last_message_at ?? report.created_at)}</p></button>; })}</div></aside>
      <section className="panel flex min-h-[34rem] flex-col overflow-hidden">
        <header className="border-b border-line bg-gradient-to-r from-ice/[0.08] via-transparent to-amber/[0.04] px-5 py-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><span className="label-mono text-amber">{selected.category}</span><span className="text-[11px] text-text-lo">via {selected.source}</span></div><h2 className="mt-2 font-display text-xl font-semibold text-text-hi">{selected.subject}</h2><p className="mt-1 flex flex-wrap items-center gap-1.5 text-xs text-text-lo"><UserRound className="size-3.5"/>{selected.email ?? selected.user_id}<span>·</span>{date(selected.created_at)}</p></div><div className="flex items-center gap-2"><span className={cn("rounded-chip border px-2.5 py-1 text-xs capitalize", selected.status === "resolved" ? "border-ok/30 bg-ok/10 text-ok" : selected.status === "in_progress" ? "border-ice/30 bg-ice/10 text-ice" : "border-amber/30 bg-amber/10 text-amber")}>{selected.status.replace("_", " ")}</span><Button size="sm" variant="ghost" onClick={() => void update(selected.status, { archived: !archived })}>{archived ? <ArchiveRestore className="size-3.5"/> : <Archive className="size-3.5"/>}{archived ? "Restore" : "Archive"}</Button></div></div></header>
        <div className="flex-1 space-y-3 overflow-y-auto p-5"><div className="flex justify-start"><div className="max-w-[82%] rounded-card bg-bg-2 px-4 py-3"><p className="whitespace-pre-wrap text-sm leading-relaxed text-text-hi">{selected.details}</p><p className="mt-2 text-[10px] text-text-lo">Member · {date(selected.created_at)}</p></div></div>{selected.messages.map((message) => { const mine = message.sender_role === "support"; return <div key={message.id} className={cn("group flex items-end gap-1", mine ? "justify-end" : "justify-start")}>{mine ? <button type="button" aria-label="Delete message" onClick={() => { if (window.confirm("Delete this support reply?")) void update(selected.status, { deleteMessageId: message.id }); }} className="invisible rounded-input p-1.5 text-text-lo hover:text-warn group-hover:visible"><Trash2 className="size-3.5"/></button> : null}<div className={cn("max-w-[82%] rounded-card px-4 py-3", mine ? "border border-ice/20 bg-ice/10" : "bg-bg-2")}><p className="whitespace-pre-wrap text-sm leading-relaxed text-text-hi">{message.body}</p><MessageAttachments media={message.media} scope="support" threadId={selected.id}/><p className="mt-2 text-[10px] text-text-lo">{mine ? "TEMPO Support" : "Member"} · {date(message.created_at)}</p></div></div>; })}</div>
