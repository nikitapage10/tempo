...
export default function AdminUserPage({ params }: { params: { id: string } }) {
  ...
  return <div className="space-y-6"><PageHeader title={user.publicProfile?.display_name ?? user.email} subtitle="Member account detail" actions={<StatusChip status={user.status} />} />
    ...
    <ConfirmDialog open={confirm === "suspend"} onOpenChange={(o) => setConfirm(o ? "suspend" : null)} title="Suspend this member?" description="Their sessions will stop and sign-in will be refused until an admin reactivates the account." confirmLabel="Suspend" busy={actions.suspend.isPending} onConfirm={suspend} />
