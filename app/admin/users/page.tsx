...
const columns: AdminColumn<AdminMember>[] = [
  { key: "member", label: "Member", className: "md:col-span-3", render: (row) => <div><Link href={`/admin/users/${row.id}`} className="font-medium text-text-hi hover:text-ice">{row.publicProfile?.display_name ?? row.email}</Link><p className="truncate text-xs text-text-lo">{row.email}</p><p className="mt-0.5 text-[11px] text-ice">{row.memberRole === "administrator" ? "Admin" : row.memberRole === "team_member" ? "Pro" : "Artist"}</p></div> },
