...
export async function GET() {
  ...
  const { data, error } = await service.from("invites").select(INVITE_COLUMNS).order("created_at", { ascending: false });
  if (error) return adminError("Couldn’t load invites.", 500);
  const memberInvites = await listMemberInvites(service).catch(() => []);
  const artistInviteRequests = await listArtistInviteRequests(service).catch(() => []);
  return adminJson({
    invites: data ?? [],
    deliveryConfig: inviteDeliveryConfig(),
    memberInvites,
    artistInviteRequests,
  });
}

export async function POST(req: NextRequest) {
  const access = await requireAdmin(); if (!access) return adminError("Forbidden.", 403);
  const body = await req.json().catch(() => null); const email = typeof body?.email === "string" && body.email.trim() ? body.email.trim().toLowerCase() : null; const note = typeof body?.note === "string" && body.note.trim() ? body.note.trim().slice(0, 500) : null; const welcomeNote = typeof body?.welcomeNote === "string" && body.welcomeNote.trim() ? body.welcomeNote.trim().slice(0, 1200) : null; const requestedRole = typeof body?.memberRole === "string" ? body.memberRole : "artist"; const memberRole = requestedRole === "team_member" || requestedRole === "administrator" ? requestedRole : "artist"; const expiresAt = typeof body?.expiresAt === "string" && body.expiresAt ? new Date(body.expiresAt).toISOString() : null; const maxUses = Math.min(100, Math.max(1, Number(body?.maxUses) || 1));
  try {
