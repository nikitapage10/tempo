...
export async function GET() {
  ...
  try {
    ...
    const [{ count: reports }, { count: supportReports }, { data: inviteRows }, { data: versions }, { data: assets }] = await Promise.all([
      service.from("content_reports").select(ID_COLUMN, { count: "exact", head: true }).eq("status", "open"),
      service.from("support_reports").select(ID_COLUMN, { count: "exact", head: true }).eq("status", "open"),
    ...
    return adminJson({
      ...
      totalStorageBytes: [...(versions ?? []), ...(assets ?? [])].reduce((sum, row) => sum + (Number(row.file_size) || 0), 0),
      signups,
    });
  } catch {
