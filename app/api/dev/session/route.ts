...
/**
 * Local-only sign-in for an existing preview identity or seeded test user.
 *
 ...
 *   3. DEV_PREVIEW_EMAIL or DEV_TEST_EMAIL must be set, and both live only in
 *      .env.local — which is gitignored and is not set in Vercel.
 *
 ...
 *
 * DEV_PREVIEW_EMAIL must name an existing account and is never auto-created.
 * DEV_TEST_EMAIL is created on first use when needed for automation.
 */
...
export async function GET(request: Request) {
  ...
  const previewEmail = process.env.DEV_PREVIEW_EMAIL?.trim();
  const testEmail = process.env.DEV_TEST_EMAIL?.trim();
  const email = previewEmail || testEmail;
  ...
  // Already signed in as the requested local identity — just continue.
  ...
  // DEV_TEST_EMAIL is allowed to seed a disposable fixture. DEV_PREVIEW_EMAIL
  // is for an existing real workspace and never creates an account on typo.
  if (!previewEmail) {
    ...
    // "already registered" is expected on every run after the first.
