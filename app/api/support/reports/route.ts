...
export async function GET(request: NextRequest) {
  ...
  if (error) return NextResponse.json({ error: "Couldn't load support conversations." }, { status: 500, headers });
  const messagePages = await Promise.all((reports ?? []).map(async (report) => {
    const { data, error: messageError } = await service.from("support_messages").select(SUPPORT_MESSAGE_COLUMNS).eq("report_id", report.id).order("created_at", { ascending: false }).order("id", { ascending: false }).limit(50);
    return { id: report.id, messages: (data ?? []).reverse(), error: messageError };
  }));
  if (messagePages.some((page) => page.error)) return NextResponse.json({ error: "Couldn't load support messages." }, { status: 500, headers });
  return NextResponse.json({ reports: (reports ?? []).map((report) => ({ ...report, messages: messagePages.find((page) => page.id === report.id)?.messages ?? [] })) }, { headers });
}
