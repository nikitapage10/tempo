    ...
    <div className="min-h-screen bg-bg-0 md:grid md:grid-cols-[15rem_minmax(0,1fr)]">
          ...
          {items.map(({ href, label, icon: Icon, attention }) => {
            ...
            return (
              <Link key={href} href={href} className={cn("flex items-center gap-3 rounded-input px-3 py-2.5 text-sm transition-colors", active ? "bg-ice/10 text-ice" : "text-text-lo hover:bg-bg-2 hover:text-text-hi")}>
                ...
                <span>{label}</span>
                {count > 0 ? <span aria-label={`${count} open ${label.toLowerCase()}`} className={cn("ml-auto min-w-5 rounded-full px-1.5 py-0.5 text-center font-mono text-[10px] font-semibold leading-4", attention === "reports" ? "bg-warn/15 text-warn" : "bg-amber/15 text-amber")}>{count > 99 ? "99+" : count}</span> : null}
              </Link>
            );
          })}
        </nav>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-7 border-t border-line bg-bg-1 md:hidden">
        {items.map(({ href, label, icon: Icon, attention }) => {
          const active = href === "/admin" ? pathname === href : pathname.startsWith(href);
          const count = attentionCount(attention);
          return <Link key={href} href={href} className={cn("flex min-w-0 flex-col items-center gap-1 px-1 py-2 text-[10px]", active ? "text-ice" : "text-text-lo")}><span className="relative"><Icon className="size-4"/>{count > 0 ? <span aria-label={`${count} open ${label.toLowerCase()}`} className={cn("absolute -right-3 -top-2 min-w-4 rounded-full px-1 text-center font-mono text-[8px] font-semibold leading-4", attention === "reports" ? "bg-warn text-bg-0" : "bg-amber text-bg-0")}>{count > 9 ? "9+" : count}</span> : null}</span><span className="truncate">{label}</span></Link>;
        })}
      </nav>
    </div>
  );
