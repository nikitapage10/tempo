...
export function StatTile({
  ...
  detail?: string;
  icon?: LucideIcon;
  tone?: "ice" | "amber" | "violet" | "ok" | "warn";
  href?: string;
}) {
  const tile = (
    <SpotlightCard tone={tone} className="block">
      <div className="panel min-h-32 p-5">
  ...
  return href ? (
    <Link href={href} className="block rounded-[var(--radius-panel)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ice">
      {tile}
    </Link>
  ) : (
    tile
