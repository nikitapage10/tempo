...
export function UsageChart({ title, detail, data, format = (value) => String(value), tone = "ice" }: { title: string; detail: string; data: { date: string; value: number }[]; format?: (value: number) => string; tone?: Tone }) {
  ...
  return <section className="panel min-w-0 overflow-hidden p-5">
    <div className="flex items-start justify-between gap-4"><div><p className="label-mono">{title}</p><p className="mt-2 text-xs text-text-lo">{detail}</p></div><div className="text-right"><p className="font-data text-xl tabular-nums text-text-hi">{format(total)}</p><p className="mt-1 text-[10px] uppercase tracking-wider text-text-lo">period total</p></div></div>
    <div className="relative mt-5 h-44 overflow-hidden rounded-input border border-line/70 bg-bg-0/40">
      {total === 0 ? <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-center"><span className="flex size-9 items-center justify-center rounded-full border border-line bg-bg-2"><BarChart3 className="size-4 text-text-lo"/></span><p className="mt-2 text-xs text-text-hi">No activity in this period</p><p className="mt-1 text-[10px] text-text-lo">The chart will build as usage is recorded.</p></div> : null}
    ...
    <div className="mt-3 flex items-center justify-between text-[10px] text-text-lo"><span>{dateLabel(start)}</span><span className="tabular-nums">Peak {format(peak)}</span><span>{dateLabel(end)}</span></div>
  </section>;
}
