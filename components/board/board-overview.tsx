        ...
        return (
          <section key={stage.id} className="relative min-w-0 overflow-hidden rounded-card border border-line bg-gradient-to-b from-[#141419] to-[#0e0e12] shadow-e1">
            ...
            <button type="button" onClick={() => onFocusStage(index)} className="relative block w-full border-b border-line/70 px-2 py-2 text-left hover:bg-white/[0.025] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ice" aria-label={`Open ${stage.name} in detailed view`}>
              <span className="block truncate font-display text-[11px] font-semibold text-text-hi sm:text-xs">{stage.name}</span>
              <span className="mt-1 flex items-center gap-1.5 font-mono text-[9px] tabular-nums text-text-lo">
                <span>{itemCount}</span>
              ...
              {tracks.slice(0, TRACK_PREVIEW_CAP).map((track) => {
                ...
                return (
                  <button key={track.id} type="button" onClick={() => onFocusStage(index)} className="flex w-full min-w-0 items-center gap-1.5 rounded-[5px] px-1 py-1 text-left hover:bg-white/[0.035] focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ice" title={track.title}>
                    ...
                    <span className="min-w-0 flex-1 truncate text-[9px] font-medium text-text-mid sm:text-[10px]">{track.title}</span>
                    {attention ? <AlertTriangle className="size-2.5 shrink-0 text-warn" /> : null}
                  </button>
                );
              })}
              {tracks.length === 0 ? <p className="px-1 py-3 text-center text-[9px] text-text-lo/45">Empty</p> : null}
              {hiddenCount > 0 ? <p className="px-1 pt-1 font-mono text-[9px] text-text-lo/55">+{hiddenCount} more</p> : null}
            </div>
          </section>
        );
      })}
