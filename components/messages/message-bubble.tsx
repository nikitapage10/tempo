...
export function MessageBubble({ message, mine, scope, threadId, authorLabel, canReact = true, canPin = true, onReply, onEdit, onDelete, onReact, onPin, onRetry }: {
  ...
  return <div className={cn("flex items-end", mine ? "justify-end" : "justify-start")}>
    <div className="group relative w-fit max-w-[80%]">
      ...
      {message.pinned ? <p className="mb-1 flex items-center gap-1 text-[11px] text-amber"><Pin className="size-2.5"/>Pinned</p> : null}
      {!mine && authorLabel ? <p className="mb-1 text-[11px] text-text-lo">{authorLabel}</p> : null}
      {message.reply_preview ? <div className="mb-2 rounded-input border-l-2 border-ice/60 bg-bg-0/30 px-2 py-1 text-xs text-text-lo">{message.reply_preview.deleted ? "Deleted message" : message.reply_preview.body || "Attachment"}</div> : null}
      ...
      {message.reactions?.length ? <div className="mt-1.5 flex flex-wrap gap-1">{message.reactions.map((reaction) => <button key={reaction.emoji} type="button" onClick={() => void onReact(message, reaction.emoji)} className={cn("rounded-chip border px-1.5 py-0.5 text-xs", reaction.reacted_by_me ? "border-ice/40 bg-ice/10" : "border-line bg-bg-0/30")}>{reaction.emoji} {reaction.count}</button>)}</div> : null}
      </div>
    </div>
  </div>;
}
