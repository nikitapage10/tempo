...
export function MessageBubble({ message, mine, scope, threadId, authorLabel, canReact = true, canPin = true, onReply, onEdit, onDelete, onReact, onPin, onRetry }: {
  ...
  return <div className={cn("group flex items-end gap-1", mine ? "justify-end" : "justify-start")}>
    <div className={cn("flex items-center gap-0.5 opacity-0 transition-opacity group-hover:opacity-100 group-focus-within:opacity-100", mine && "order-first")}>
    ...
    <div className={cn("relative max-w-[80%] rounded-card px-3 py-2 text-sm", mine ? "bg-ice/15 text-text-hi" : "bg-bg-2 text-text-hi", deleted && "border border-dashed border-line bg-transparent text-text-lo")}>
