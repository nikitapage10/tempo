...
export function MessageComposer({ scope, threadId, onSend, pending, compact = false, placeholder = "Write a message...", draftKey, replyTo, onCancelReply, onTyping }: Props) {
  ...
  return <div className={cn("space-y-2", compact && "space-y-1.5")}
    ...
    {files.length ? <div className="flex flex-wrap gap-1.5">{files.map(({ file }, index) => <span key={`${file.name}-${index}`} className="inline-flex max-w-full items-center gap-1 rounded-chip border border-line bg-bg-2 px-2 py-1 text-[11px] text-text-lo"><FileText className="size-3"/><span className="max-w-40 truncate">{file.name}</span><button type="button" aria-label={`Remove ${file.name}`} onClick={() => setFiles((current) => current.filter((_, item) => item !== index))}><X className="size-3"/></button></span>)}</div> : null}
    ...
    {desktop || working ? <div className="flex min-h-6 items-center gap-2">
      {desktop && devices.length ? <label className="flex items-center gap-1 text-[11px] text-text-lo"><span>Mic</span><select value={deviceId ?? ""} onChange={(event) => setDeviceId(event.target.value || null)} className="max-w-44 rounded-input border border-line bg-bg-1 px-1.5 py-1 text-[11px] text-text-hi"><option value="">System default</option>{devices.map((device, index) => <option key={device.deviceId} value={device.deviceId}>{device.label || `Microphone ${index + 1}`}</option>)}</select></label> : null}
      {working ? <p className="text-[11px] text-text-lo">{files.length ? `Uploading${uploadProgress !== null ? ` ${uploadProgress}%` : ""} and sending...` : "Working..."}</p> : null}
      {working && files.length ? <button type="button" onClick={() => uploadAbortRef.current?.abort()} className="text-[11px] text-warn">Cancel upload</button> : null}
    </div> : null}
  </div>;
}
