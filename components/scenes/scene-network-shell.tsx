...
export function SceneNetworkShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-bg-0 text-text-hi">
    ...
    <div className="mx-auto grid max-w-[1520px] md:grid-cols-[220px_minmax(0,1fr)]"><aside className="sticky top-16 hidden h-[calc(100vh-4rem)] border-r border-line p-4 md:block"><p className="label-mono px-3 py-2">Your network</p><nav className="mt-2 space-y-1">{items.map(({ href, label, icon: Icon }) => { const base = href.split("?")[0]; const active = base === "/scene" ? pathname === "/scene" : pathname.startsWith(base); return <Link key={href} href={href} className={cn("flex items-center gap-2.5 rounded-input px-3 py-2 text-sm", active ? "bg-bg-2 text-ice" : "text-text-lo hover:bg-bg-2/60 hover:text-text-hi")}><Icon className="size-4" />{label}</Link>; })}</nav><div className="absolute inset-x-4 bottom-5 rounded-panel border border-line bg-bg-1 p-4"><Settings2 className="size-4 text-amber" /><p className="mt-3 text-xs font-medium text-text-hi">Independent by design</p><p className="mt-1 text-[11px] leading-5 text-text-lo">Scenes can run without an artist workspace.</p></div></aside><main className="min-w-0 px-4 py-6 sm:px-6 lg:px-8">{children}</main></div>
  </div>;
}
