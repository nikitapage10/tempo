...
export function useAdminSupport(status: string) { const client = useQueryClient(); const query = useQuery({ queryKey: ["admin", "support", status], queryFn: () => getAdminSupport(status) }); const update = useMutation({ mutationFn: ({ id, nextStatus, adminNotes }: { id: string; nextStatus: "open" | "in_progress" | "resolved"; adminNotes?: string }) => updateAdminSupport(id, { status: nextStatus, adminNotes }), onSettled: () => Promise.all([client.invalidateQueries({ queryKey: ["admin", "support"] }), client.invalidateQueries({ queryKey: ["admin", "audit"] })]) }); return { ...query, update }; }
