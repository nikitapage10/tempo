...
export function useAdminSupport(status: string) { const client = useQueryClient(); const query = useQuery({ queryKey: ["admin", "support", status], queryFn: () => getAdminSupport(status) }); const update = useMutation({ mutationFn: ({ id, nextStatus, adminNotes, reply }: { id: string; nextStatus: "open" | "in_progress" | "resolved"; adminNotes?: string; reply?: string }) => updateAdminSupport(id, { status: nextStatus, adminNotes, reply }), onSettled: () => Promise.all([client.invalidateQueries({ queryKey: ["admin", "support"] }), client.invalidateQueries({ queryKey: ["admin", "overview"] }), client.invalidateQueries({ queryKey: ["admin", "audit"] })]) }); return { ...query, update }; }
