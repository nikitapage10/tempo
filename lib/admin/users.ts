...
export async function memberAggregates(userIds: string[]) {
  ...
  if (!userIds.length) return { profiles: new Map<string, PublicProfile>(), flags: new Map<string, Flag>(), tracks: new Map<string, number>(), projects: new Map<string, number>(), storage: new Map<string, number>() };
  const [profilesResult, flagsResult, tracksResult, projectsResult, versionsResult] = await Promise.all([
  ...
  return { profiles, flags, tracks, projects, storage };
}
...
export async function getInviteForUser(userId: string) {
  ...
  return invite ? { code: invite.code, redeemedAt: redemption.redeemed_at } : null;
}
