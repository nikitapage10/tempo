...
type Onboarding = { user_id: string; eligible: boolean; started_at: string; main_tour_completed_at: string | null; checklist_steps: string[]; checklist_dismissed_at: string | null; checklist_completed_at: string | null; page_tours_completed: string[]; welcome_message_sent_at: string | null; last_seen_at: string };
...
export async function memberAggregates(userIds: string[]) {
  ...
  if (!userIds.length) return { profiles: new Map<string, PublicProfile>(), flags: new Map<string, Flag>(), tracks: new Map<string, number>(), projects: new Map<string, number>(), storage: new Map<string, number>(), onboarding: new Map<string, Onboarding>() };
  const [profilesResult, flagsResult, tracksResult, projectsResult, versionsResult, onboardingResult] = await Promise.all([
    ...
    service.from("member_onboarding").select("user_id, eligible, started_at, main_tour_completed_at, checklist_steps, checklist_dismissed_at, checklist_completed_at, page_tours_completed, welcome_message_sent_at, last_seen_at").in("user_id", userIds),
  ]);
  ...
  return { profiles, flags, tracks, projects, storage, onboarding };
}
...
export async function getInviteForUser(userId: string) {
  ...
  return invite ? { code: invite.code, memberRole: invite.member_role ?? "beta_artist", redeemedAt: redemption.redeemed_at } : null;
}
