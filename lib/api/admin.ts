...
export type AdminMember = { id: string; email: string; createdAt: string; lastSignInAt: string | null; provider: string; status: "active" | "suspended"; publicProfile: { id: string; handle: string | null; display_name: string; visibility: string } | null; trackCount: number; projectCount: number; storageBytes: number; onboarding: AdminOnboardingSummary | null };
...
export type AdminInviteRole = "beta_artist" | "team_member" | "administrator";
...
export type AdminSupportMessage = { id: string; report_id: string; sender_role: "member" | "support"; sender_user_id: string | null; body: string; media: MessageAttachment[]; deleted_at: string | null; created_at: string };
