...
import { buildMessageMediaPath, uploadFile } from "@/lib/storage";
...
export async function uploadMessageAttachments(files: File[], scope: "direct" | "support", threadId: string): Promise<MessageAttachment[]> {
...
export async function getMessageAttachmentUrl(input: { scope: "direct" | "support"; threadId: string; path: string }) {
