...
- Board (/board): mode-aware. In a music-focused artist Space it is Kanban by song stage; populated desktop columns scroll horizontally and phone columns stack. Stage + adds an existing unplaced track, a new track, or a sticky note. In a Pro Space it is a professional workflow Board using the same tasks as Tasks, with To do / In progress / Done columns, drag-to-update status, quick add, and category/project/search filters. A Pro Board never shows songs or changes an artist pipeline.
...
- Messages (/messages): artist direct messages, artist group chats, and a separate private TEMPO Support inbox. The global top-right Messages button shows unread threads and a quick-reply mini-inbox. New message can start a 1:1 thread or a group chat with several artists. Full conversations support archive/restore, sender-only deletion, dictation, and up to four private attachments. Support works even when the artist stays off the social network; support staff never appear as an artist profile.
