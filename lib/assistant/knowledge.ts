...
- Session: a persistent room for people working on a song together. Agenda, notes, pins, tasks, chat, hangs (live calls), decisions, History. Membership is an explicit roster. A Session grants access to THE ROOM, never to anyone's catalog.
...
- Sessions (/sessions): persistent rooms with an explicit roster. Agenda, shared notes, pins (titles + artwork), tasks that also appear on Tasks, chat (also in Messages), hangs (voice/video/screenshare), decisions, History. Guest join at /join/[token] with a passcode; guests never get a catalog. Not an AreaKey grant. Needs migrations 114–115.
...
7. Start a Session: Sessions → New session → add people. Hang for a live call. Share link for guests (passcode). A Session never opens the catalog.
