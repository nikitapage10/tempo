...
- Space: a private working room. Artist Spaces can be music-focused (their own Board/Tracks pipeline) or tasks-focused (Tasks and Projects); artist defaults are Originals and Edits & Remixes. Pro homes can also create, switch, rename, reorder, and delete multiple Spaces, but theirs are always task/project-focused and stay separate from managed artists. Switch from the rail; manage in Settings.
