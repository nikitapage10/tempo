/**
 * "Waves" flow-field shader — made with the 21st.dev Shader Builder.
 *
 * Standalone from lib/shader-glsl.ts (the root Spectra lightfield): this one
 * is a self-contained, per-surface backdrop with its own palette, mounted
 * only where a page asks for it. See components/waves-backdrop.tsx.
 *
 * The fragment shader packs its parameters into seven vec4s plus the colour
 * array — 15 fragment uniform vectors, one below WebGL1's guaranteed minimum
 * of 16 — and exposes readable `u_*` names as macros over them. Feed the
 * packed vectors; read the macro names.
 */

export const WAVES_VERTEX_SHADER = /* glsl */ `
...
export const WAVES_FRAGMENT_SHADER = /* glsl */ `
...
/**
 * The "Waves" recipe, exactly as tuned in the builder. Values are the packed
 * uniform vectors; the two slots left out here are filled per frame
 * (u_scene gets resolution + time, u_space gets the pointer).
 */
export const WAVES_RECIPE = {
  /** Low → high. Deep sea, teal, seafoam, sand. */
  colors: [
    [0.0, 0.071, 0.098],
    [0.0, 0.373, 0.451],
    [0.58, 0.824, 0.741],
  ...
  /** Multiplier on elapsed seconds — "speed 60/100". */
  timeScale: 1.24,
  /** scale, intensity, paramA, warp */
  shape: [1.92, 0.6, 0.33, 0.26] as const,
  /** detail, contrast, brightness, saturation */
  surface: [3.39, 1.15, 0.0, 1.0] as const,
  /** hue, vignette, blur, grain */
  finish: [0.0, 0.2, 0.002, 0.07] as const,
  /** seed, rotation, drift, OKLab toggle */
  transform: [8231.0, 4.19, 0.15, 0.0] as const,
  /** offset.xy — pointer.zw is appended per frame */
  offset: [-0.05, -0.13] as const,
  /** presence, effect, strength, radius. Presence 0 = cursor interaction off. */
  cursor: [0.0, 2.0, 0.65, 0.46] as const,
};
