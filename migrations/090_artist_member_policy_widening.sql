...
--
-- Every policy added below is a NEW, additional permissive policy — nothing
-- existing is dropped or replaced. Postgres OR's permissive policies for the
-- same command together, so a team member's access is purely additive on
-- top of the owner's; there is no way this migration can narrow what an
-- artist owner can already do.
--
